...
class Omexer_Recent_Post_Widget extends WP_Widget {
    public function __construct(){
        parent::__construct('omexer-insight-latest-post', esc_html__('Advanced Recent Posts', 'omexer-insight'), array(
            'description'   =>  esc_html__('latest post with featured image', 'omexer-insight'),
        ));
    }
    ...
    public function form($instance){
        $title = ! empty($instance['recent_post_title']) ? $instance['recent_post_title'] : esc_html__(' Recent Posts', 'omexer-insight');
        $post_count = ! empty($instance['post_count']) ? $instance['post_count'] : 3;
        ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('recent_post_title')); ?>"><?php echo esc_html__('Title :', 'omexer-insight') ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('post_count')); ?>"><?php echo esc_html__('Post Count:', 'omexer-insight') ?></label>
...
if ( ! function_exists( 'omexer_insight_latest_post' ) ) {
	function omexer_insight_latest_post() {
		register_widget('Omexer_Recent_Post_Widget');
	}

	add_action('widgets_init', 'omexer_insight_latest_post');
}